...
export default async function newComment(
  ...
  try {
    ...
    return IO.ifLeft((data) => fail(400, response, data)).ifRight(() =>
...
const createNewComment = (newCom: AddComment): EitherAsync<string, Comment> =>
  EitherAsync<string, AddComment>(async ({ liftEither }) => {
    ...
    .chain((decodedComment) =>
      EitherAsync<string, firestore.DocumentReference<firestore.DocumentData>>(
        async ({ fromPromise }) => {
    ...
    .chain((queryResult) =>
      EitherAsync(async ({ liftEither }) => {
        const decodedResult = await liftEither(
          await decodeComment(queryResult)
        );
...
const addCommentIO = async (
  ...
  Either<string, firestore.DocumentReference<firestore.DocumentData>>
> => {
  try {

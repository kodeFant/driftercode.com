...
import { sendCommentNotification } from "../email/commentNotification";
...
export default async function commentApproval(
  request: Request,
  response: Response
) {
  try {
...
const commentApprovalIO = (
  request: Request
): EitherAsync<string, FirebaseFirestore.WriteResult> =>
  EitherAsync<string, UpdateApprovalBody>(async ({ liftEither }) => {
    const body = await liftEither(
      UpdateApprovalBody.decode({ commentId: request.params.commentId })
    );
    return body;
  })
    .chain((body) =>
    ...
    .chain((commentRef) =>
      EitherAsync(async ({ fromPromise, liftEither }) => {
        ...
        const commentData = await (await commentRef.get()).data();

        const comment = await liftEither(
          Comment.decode({ ...commentData, id: "placeholder" })
        );
        await fromPromise(

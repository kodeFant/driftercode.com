...
One thing we need to cover for the TypeScript app to behave like a real Elm app is decoding incoming data.
...
Elm actually forces you to do that, and that contributes to eliminating pretty much all runtime exceptions. When something doesn't check out, if fails fast and loud.
...
In this series we will use [Purify](https://gigobyte.github.io/purify/). I prefer it because it's a small simple library that gives us functional data structures and does it well.

[fp-ts](https://gcanti.github.io/fp-ts/) is a powerful library, but I find it harder to learn and the documentation is not as beginner friendly.
...
If you need something even more functional and safe, just take the [leap of faith](/blog/my-first-steps-into-functional-programming) and move on to [Elm](https://elm-lang.org/) already. 
...
As mentioned, it does not help us checking external data. And a puny TypeScript typeguard is too weak to withstand the chaos outside your source code. 
...
What 😱 Who wrote that error message??
...
Another improvement this app needs to resemble Elm is immutable state. A simple way is to use the [useImmer hook](https://github.com/immerjs/use-immer) instead of useState.
...
But to get the full benefits and power of Elm, it's best to use Elm. Try it out on a small component in your app and decide wether you want to expand it.
